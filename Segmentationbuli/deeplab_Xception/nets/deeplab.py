...
def Deeplabv3(input_shape=(512, 512, 3), classes=21, alpha=1.,OS=16):
    ...
    img_input = Input(shape=input_shape) # b,512,512,3

    # x         64, 64, 2048
    # skip1     128, 128, 256
    x, atrous_rates, skip1 = Xception(img_input,alpha,OS=OS)
    size_before = tf.keras.backend.int_shape(x) # 返回张量或变量的shape，作为int或None条目的元组

    #---------------------------------------------------------------#
    #   全部求平均后，再利用expand_dims扩充维度
    #   64,64,2048 -> 1,1,2048 -> 1,1,2048
    #---------------------------------------------------------------#
    b4 = GlobalAveragePooling2D()(x) # b,64,64,2048 -> b,2048
    b4 = Lambda(lambda x: K.expand_dims(x, 1))(b4) #为何不直接 b4=K.expand_dims(b4, 1)
    b4 = Lambda(lambda x: K.expand_dims(x, 1))(b4) # b,1,1,2048
    b4 = Conv2D(256, (1, 1), padding='same', use_bias=False, name='image_pooling')(b4)# b,1,1,256
    ...
    #   调整通道
    #---------------------------------------------------------------#
    b0 = Conv2D(256, (1, 1), padding='same', use_bias=False, name='aspp0')(x) #    64, 64, 256
    ...
    x = Lambda(lambda xx: tf.image.resize_images(x, skip1.shape[1:3]))(x)  #这里resize相当于上采样了
                                                   
    # 128,128,24 -> 128,128,48  这里开始处理低维度的feature map
    ...
    # 128,128,256 -> 128,128,21 -> 512,512,21 这里开始上采样
    x = Conv2D(classes, (1, 1), padding='same')(x) # 128,128,21
    
    size_before3 = tf.keras.backend.int_shape(img_input) #返回shape int类型元组 b,512,512,3
    x = Lambda(lambda xx:tf.image.resize_images(xx,size_before3[1:3]))(x) # b,512,512,3

    x = Reshape((-1,classes))(x)  #暴力还原21 b,512,512,21
    x = Softmax()(x)  #  换成概率
